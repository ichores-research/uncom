...
class UnderstandingNode:
    def __init__(self, output_dir=None, device = 'auto'):
        ...
        rospy.init_node('understanding_node', anonymous=True)
        ...
        self.video_filename = "input.avi"
        self.audio_filename = "temp.wav"
        self.device = device
        ...
        if detection_status:
            ...
            if self.speech_detected:
                ...
                if self.silence_time >= self.max_silence:
                    ...
                    self.publish_save_video(self.output_dir / self.video_filename)
                    self.publish_save_audio(self.output_dir / self.audio_filename)
                    understood = self.understand(self.device,
                                    self.output_dir,
                                    self.audio_filename,
                                    self.video_filename)
                    if understood[0] == "incomplete":
                        pass
                    elif understood[0] == "ambiguous":
                        self.request_repeat()
                    elif understood[0] == "OK":
                        self.execute()
                    else:
    ...
    def publish_clear_video(self):
        ...
        msg = Empty()
        msg.data = file_path
        self.clear_video_pub.publish(msg)
    ...
    #    hand_detector = PointingDetector()
        ...
        if not transcriber.command_check(transcription['text']): return ["incomplete"]    
...
from nltk.corpus import stopwords
        ...
        return ["OK", "object", [1,2,3], "action", "destination", [1,2,3]]
...
if __name__ == '__main__':
    ...
    node.understand()  # Call the dummy understand function to simulate publishing
    node.run()
