...
import torch
from uncom_docker.uncom_utils.audio import AudioTranscriber, separate_audio
from uncom_docker.uncom_utils.image import (
    PointingDetector,
    ObjectDetector,
    Segmenter,
    DetectionResult,
...
from uncom_docker.uncom_utils.text import CommandExtractor, check_relative_position
...
#    hand_detector = PointingDetector()
    ...
    command_path = output_dir / f"{video_name}_command.json"
    ...
    if target_concrete:   
        ...
        if area_target: # if no objects are detected, target is an empty space; case 4)
            ...
            depth_estimator.render_depth(depths, output_dir)
    ...
    annotated_object_image_path = output_dir / f"{video_name}_{final_time}_annotated_object.png"
    ...
    annotated_target_image_path = output_dir / f"{video_name}_{final_time}_annotated_target.png"
    ...
    annotated_action_path = output_dir / f"{video_name}_{final_time}_annotated_action.png"
...
if __name__ == "__main__":
    ...
    total_tries = 0
    success = 0 
    video_total = os.listdir(video_folder)
    num_video = len(video_total)
    analyzed_files = os.listdir(args.output_dir)
    for i, video in enumerate(video_total):
        print(video, i/num_video*100, "%")
        if ".mp4" in video:
            total_tries += 1
            try:
                result = understand(f"{video_folder}{video}", args.output_dir, args.device)
                if result: 
                    success +=1
            except Exception as e:
                print(f"Understanding for file {video} failed with error: {e}")
    success_rate = total_tries/success
